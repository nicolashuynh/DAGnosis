...
def is_dag(W: np.ndarray) -> bool:
    ...
    is_dag = G.is_dag()
    assert isinstance(is_dag, bool), "is_dag should return a boolean"
    return is_dag


def find_parent_size(DAG: np.ndarray, feature: int) -> int:
    """Find the number of parents of a feature in a DAG."""
...
def simulate_dag(d: int, s0: int, graph_type: "str") -> np.ndarray:
    ...
    B_perm: np.ndarray = _random_permutation(B)
...
def generate_SEM(sem_type: str, pa_size: int) -> Tuple[Callable, dict]:
    parameters: Dict[str, Union[FloatArray, float]] = {}
    if pa_size == 0:
        return lambda X, z: z, {}
    if sem_type == "mlp":
        hidden = 100
...
def generate_list_SEM(B: np.ndarray, sem_type: str) -> Tuple[list, list]:
    d = B.shape[0]
...
def simulate_sem_by_list(
    B: np.ndarray,
    n: int,
    list_SEMs: list,
    noise_scale: Optional[list] = None,
