...
def is_dag(W: np.ndarray) -> bool:
    ...
    G_is_dag = G.is_dag()
    assert isinstance(G_is_dag, bool)
    return G_is_dag
...
def simulate_dag(d: int, s0: int, graph_type: str) -> np.ndarray:
    ...
    assert isinstance(B_perm, np.ndarray)
    return B_perm


def simulate_parameter(
    B: np.ndarray,
    w_ranges: typing.Tuple = ((-2.0, -0.5), (0.5, 2.0)),
) -> np.ndarray:
    r"""
...
def simulate_linear_sem(
    ...
    if noise_scale is None:
        ...
        assert isinstance(noise_scale, float), "noise scale must be a scalar or a list"
        scale_vec = noise_scale * np.ones(d)
    else:
        assert isinstance(noise_scale, list), "noise scale must be a scalar or a list"
        if len(noise_scale) != d:
            raise ValueError("noise scale must be a scalar or have length d")
        scale_vec = np.array(noise_scale)
    if not is_dag(W):
...
def simulate_nonlinear_sem(
    ...
    n: int,
    sem_type: str,
    noise_scale: typing.Optional[typing.List[float]] = None,
) -> np.ndarray:
    r"""
