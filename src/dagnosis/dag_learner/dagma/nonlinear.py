...
class DagmaNonlinear:
    ...
    def __init__(
        self, model: DagmaMLP, verbose: bool = False, dtype: torch.dtype = torch.double
    ):
        """
        Parameters
        ----------
    ...
    def fit(
        ...
        mu_init: float = 0.1,
        mu_factor: float = 0.1,
        s: Union[float, list, int] = 1.0,
        warm_iter: int = int(5e4),
        max_iter: int = int(8e4),
        lr: float = 0.0002,
        ...
        assert isinstance(W_est, np.ndarray)
        return W_est
