...
class DagmaLinear:
    ...
    def __init__(
        self, loss_type: str, verbose: bool = False, dtype: type = np.float64
    ) -> None:
        r"""
        Parameters
        ----------
        ...
        self.opt_m: np.ndarray = np.array([])
        self.opt_v: np.ndarray = np.array([])
    ...
    def _adam_update(
        self, grad: np.ndarray, iter: int, beta_1: float, beta_2: float
    ) -> np.ndarray:
        r"""
    ...
    def minimize(
        ...
        """
        obj_prev = 1e16
        self.opt_m = np.zeros_like(W, dtype=self.dtype)
        self.opt_v = np.zeros_like(W, dtype=self.dtype)
        self.vprint(
    ...
    def fit(
        ...
        self.Id: np.ndarray = np.eye(self.d).astype(self.dtype)
        ...
        if exclude_edges is not None:
            if (
                type(exclude_edges) is tuple
                and type(exclude_edges[0]) is tuple
        ...
        if include_edges is not None:
            if (
                type(include_edges) is tuple
                and type(include_edges[0]) is tuple
        ...
        self.W_est: np.ndarray = np.zeros((self.d, self.d)).astype(
            self.dtype
        ...
        assert isinstance(s, list), "s should be a list"
...
def test():
    ...
    n, d, s0 = (
        ...
        20,
        20,
    )  # the ground truth is a DAG of 20 nodes and 20 edges in expectation
